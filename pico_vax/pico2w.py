"""
VAX/VMS Emulator for Raspberry Pi Pico 2 W
WiFi-enabled version with TCP/IP telnet access.

Features:
- WiFi connectivity for remote terminal access
...
TCP_PORT = 2323  # Port to listen on (23 for standard telnet, 2323 to avoid root)
...
class SerialTerminal:
    """Serial terminal handler for local/USB access."""
    __slots__ = ('running', 'history')
...
class NetworkTerminal:
    ...
    __slots__ = ('socket', 'addr', 'running', 'history', 'recv_buffer')
    ...
    def read_line(self):
        ...
        while self.running:
            byte = self._recv_byte(timeout_ms=30000)
...
class VAXVMS:
    ...
    def cmd_show(self, args, quals):
        ...
        if subcmd == 'DEFAULT':
            ...
            self.terminal.writeln("  Uptime: {} seconds".format(int(time.time())))
    ...
    def _show_network(self):
        ...
        self.terminal.writeln("  VAX/VMS Network Status")
        self.terminal.writeln("")
        if MICROPYTHON:
            try:
                ...
                if wlan.isconnected():
                    ...
                    self.terminal.writeln("  Network:         WiFi (Connected)")
                    self.terminal.writeln("  IP Address:      {}".format(config[0]))
                    self.terminal.writeln("  Subnet Mask:     {}".format(config[1]))
                    self.terminal.writeln("  Gateway:         {}".format(config[2]))
                    self.terminal.writeln("  DNS Server:      {}".format(config[3]))
                    self.terminal.writeln("  TCP Port:        {}".format(TCP_PORT))
                else:
                    self.terminal.writeln("  Network:         WiFi (Not Connected)")
            except:
                self.terminal.writeln("  Network:         Not Available")
        else:
            self.terminal.writeln("  Network:         Loopback Only")
            self.terminal.writeln("  IP Address:      127.0.0.1")

    def cmd_tcpip(self, args, quals):
        """TCPIP command - show network info."""
        self._show_network()
    ...
    def cmd_create(self, args, quals):
        ...
        if 'DIRECTORY' in quals or 'DIR' in quals:
            ...
            name = args[0].upper() if args else "NONAME.DAT"
            if '.' not in name:
                name += '.DAT'
    ...
    def cmd_help(self, args, quals):
        ...
        if args:
            ...
            help_topics = {
                ...
                'SHOW': "SHOW DEFAULT|TIME|SYSTEM|USERS|MEMORY|DEVICES|SYMBOL|PROCESS|QUOTA|NETWORK",
                ...
                'TCPIP': "TCPIP - Show network configuration\nSHOW NETWORK - Same as TCPIP",
            }
        ...
        self.terminal.writeln("Network Commands:  TCPIP, SHOW NETWORK")
...
class VMSServer:
    """TCP server that handles VMS sessions over WiFi."""
    ...
    def start(self):
        ...
        try:
            ...
            print("\n" + "=" * 50)
            print("VAX/VMS TCP Server Started")
            print("=" * 50)
    ...
    def run(self):
        """Main server loop - accepts connections and spawns VMS sessions."""
        if not self.running:
            if not self.start():
                return
...
def main_serial():
    """Run VMS with serial/USB terminal (original behavior)."""
    vms = VAXVMS()
    vms.boot()


def main_wifi():
    """Run VMS as a WiFi TCP server."""
    server = VMSServer(TCP_PORT)
    server.run()
...
def main():
    ...
    if MICROPYTHON:
        print("Starting VAX/VMS WiFi Server...")
