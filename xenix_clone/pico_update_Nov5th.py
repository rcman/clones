...
class SerialTerminal:
    ...
    UART0 pins: TX=GPIO0, RX=GPIO1
    UART1 pins: TX=GPIO4, RX=GPIO5 (or GPIO8, RX=GPIO9)
    
    Supported baud rates: 300, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200
    """
    
    # Common baud rates for retro terminals
    BAUD_RATES = [300, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200]
    
    def __init__(self, use_uart=False, uart_id=0, baud_rate=115200, tx_pin=0, rx_pin=1):
        ...
        if use_uart:
            try:
                ...
                print(f"UART{uart_id} initialized: {baud_rate} baud, TX=GPIO{tx_pin}, RX=GPIO{rx_pin}")
            except Exception as e:
                print(f"UART init failed: {e}")
                self.use_uart = False
    
    def set_baud_rate(self, baud_rate):
        """Change UART baud rate on the fly"""
    ...
    def read_char(self):
        """Read a single character from UART or stdin"""
        if self.use_uart and self.uart:
            if self.uart.any():
                return self.uart.read(1).decode('utf-8', 'ignore')
            return None
        else:
            # Non-blocking read for stdin
            import select
    ...
    def read_line(self):
        """Read a line of input with proper terminal handling"""
        line = ""
        while True:
            # Read character based on interface
            if self.use_uart and self.uart:
                # UART mode: blocking read
                if self.uart.any():
...
class XenixOS:
    def __init__(self, use_uart=False, uart_id=0, baud_rate=115200, tx_pin=0, rx_pin=1):
        ...
        self.hostname = "pico"
        self.terminal = SerialTerminal(use_uart, uart_id, baud_rate, tx_pin, rx_pin)
        self.running = True
    ...
    def cmd_stty(self, args):
        """Display terminal settings (Unix stty command)"""
...
def main():
    ...
    For USB serial (default):
        os = XenixOS()
    
    For UART0 (GPIO 0/1) at 115200 baud:
        os = XenixOS(use_uart=True, uart_id=0, baud_rate=115200, tx_pin=0, rx_pin=1)
    
    For UART1 (GPIO 4/5) at 9600 baud (classic terminal):
        os = XenixOS(use_uart=True, uart_id=1, baud_rate=9600, tx_pin=4, rx_pin=5)
    
    For UART0 at 2400 baud (retro terminal):
        os = XenixOS(use_uart=True, uart_id=0, baud_rate=2400, tx_pin=0, rx_pin=1)
    """
    
    # Default: USB serial mode
    os = XenixOS()
    
    # Uncomment for UART terminal on GPIO 0/1:
    # os = XenixOS(use_uart=True, uart_id=0, baud_rate=115200, tx_pin=0, rx_pin=1)
    
    # Uncomment for classic 9600 baud terminal:
    # os = XenixOS(use_uart=True, uart_id=0, baud_rate=9600, tx_pin=0, rx_pin=1)
